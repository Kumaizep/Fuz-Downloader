...
def try_login(driver, account):
	...
	signin_buttom = driver.find_element(By.CSS_SELECTOR, "button[class^='signin_form']")
...
def get_select_result(min_id, max_id):
	...
	while need_select:
		try:
			result = list(map(int, input("どの本をダウンロードしますか？(数字のみ、スペースで区切る。例：1 0 3) ").split()))
			need_select = False
			for i in result:
...
def other_selector():
	...
	while need_select:
		try:
			result = list(map(int, input("ダウンロードしたい本のURLの末尾にある番号を入力してください。\n (例えば、「ご注文はうさぎですか？１巻 第０話」の場合は、「2443」を入力してください。スペースで区切る。) \n").split()))
			need_select = False
			for i in result:
				if i < 0:
					print(bcolors.WARNING, "[!] 無効な選択：", str(i), bcolors.ENDC)
...
def init_book(driver):
	page = driver.find_element(By.CSS_SELECTOR, "p[class^='ViewerFooter_footer__page']")
...
def get_bookmarks(driver):
	...
	catalog.click()
	dailogs_name = driver.find_elements(By.CSS_SELECTOR, "p[class^=ViewerIndexModal_dialog__name]")
	dailogs_index = driver.find_elements(By.CSS_SELECTOR, "p[class^=ViewerIndexModal_dialog__index]")
